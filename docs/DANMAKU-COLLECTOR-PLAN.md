...
| **R16** | **监听源与场次解耦**（D1 · 大票） | 拆出独立「监听源」资源：可只监听、不落库、不绑直播；落库时才决定归属。涉及 `services/liveCollector.ts`、`routes/danmakuSource.ts`、App 工作台采集卡 | R13 | **不依赖开播也能贴链接监听并看弹幕流水**（用户本次的实际诉求）；开播路径行为不变 | 🟡 **服务端已完成**（`liveCollector.startMonitor/stopMonitor/monitorEvents/listMonitors` + `routes/danmakuWatch.ts` 四个端点 + 5 例测试，事件只进内存环形缓冲**不落库**）；**App 侧入口待做 → R16b** |

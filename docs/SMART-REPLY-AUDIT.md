...
## 修复记录（2026-09-17 · 提交 008800a）

| 问题 | 状态 | 怎么修的 |
|---|---|---|
| **§1 频控竞态** 🔴 | ✅ **已修** | 新增 `SerialByKey`：**每场次一条串行链**。频控的 check-then-act 被串行化，竞态从根上消失；顺带把「同一场次并发调 AI 数」压到 1。**实测：并发 10 条从 10 次 AI 调用降到 1 次**（同一探针，修前修后对比）。粒度是**场次**不是全局 —— 不同场次互不阻塞（有单测）。 |
| §2.1 频控 Map 无界增长 🟠 | ✅ **已修** | 新增 `forgetLive(liveId)`，由结束直播的收尾路径（`finishLive`）调用；另加 **500 条上界安全网**（异常路径下也不会涨）。 |
| §2.3 异常被静默吞掉 🟠 | ✅ **已修** | `subscribe` 的 `.catch` 改为**落日志**（带 liveId）。引擎是 fire-and-forget 调的，不记日志就完全没痕迹。 |
| §2.2 回复台账只在开播清 🟠 | ⬜ 未修 | 见 R29 余项 |
| §2.4 DeepSeek 无重试 🟠 | ⬜ 未修 | R31 |
| §2.5 出声队列无上限 🟠 | ⬜ 未修 | 与 R31 一起 |
| §2.6 匿名绕过用户级频控 🟡 | ⬜ 未修 | 低优先 |
| §2.7 senderIntervalMs 硬编码 🟡 | ⬜ 未修 | 低优先 |
| §2.8 每条弹幕 2 次 DB 🟡 | ⬜ 未修 | 低优先 |

**回归防护**：`interaction_engine.test.ts` 新增 3 条 —— 并发 10 条只 1 次 AI 调用 / 两场次各自并发互不拖累 / `forgetLive` 清完立刻可再回。

> ⚠️ 修完这条，**「并发下频控失效」这个最严重的问题没有了**。§2.4/§2.5 仍建议在长时间无人值守前处理。

## 1. 🔴 严重：频控竞态 —— 已实测复现（**已修，见上**）
